...
class DatabaseService:
    ...
    async def update_guild_settings(self, guild_id: str, submission_days: int = None, 
                             voting_days: int = None, channel_id: str = None) -> Guild:
    ...
    async def update_player_score(self, guild_id: str, user_id: str, score_to_add: int) -> Player:
    ...
    async def get_leaderboard(self, guild_id: str, limit: int = 5) -> list[Player]:
        ...
        query = select(Player).where(Player.guild_id == guild.id).order_by(Player.total_score.desc()).limit(limit)
        result = await self.session.execute(query)
    ...
    async def complete_round(self, round_id: int, results_message_id: str = None) -> Round:
    ...
    async def update_round_message_ids(self, round_id: int, submission_message_id: str = None, 
                                     voting_message_id: str = None) -> Round:
    ...
    async def update_round_timing(self, round_id: int, submission_end: datetime = None, 
                               voting_end: datetime = None) -> Round:
    ...
    async def create_submission(self, guild_id: str, user_id: str, content: str, description: str = None) -> Submission:
