...
class SettingsCog(commands.Cog):
    ...
    @app_commands.command(name="settings", description="Configure Music League settings")
    ...
    async def settings(self, interaction: discord.Interaction, 
        ...
        if not interaction.user.guild_permissions.manage_guild:
            await interaction.response.send_message("You need 'Manage Server' permission to change settings.", ephemeral=True)
            return
        ...
        async with self.bot.get_db_session() as session:
            ...
            embed.add_field(
                name="Voting Period", 
                value=f"{updated_settings.voting_days} days"
            )
    ...
    @app_commands.command(name="leaderboard", description="Show the top players in Music League")
    ...
    async def leaderboard(self, interaction: discord.Interaction, limit: int = 5):
        ...
        async with self.bot.get_db_session() as session:
            ...
            if not top_players:
                await interaction.response.send_message("No players in the leaderboard yet!")
                return
            
            embed = discord.Embed(
                title="🏆 Music League Leaderboard 🏆",
